...
daysGerman = ["Montag","Dienstag","Mittwoch","Donnerstag","Freitag","Samstag","Sonntag"]
...
datespan_regex = re.compile('(?P<from>\d+\.\d+\.\d{0,4})\s*–\s*(?P<to>\d+\.\d+\.\d{0,4})')
...
def parse_url(url, today=False):
    ...
    try:
        ...
        employee = float(p["employee"].split(",")[0]) + float(p["employee"].split(",")[1])/100
    ...
    # Date
    p = datespan_regex.search(document.find("div", {"class": "maincontent"}).find("h2").text).groupdict()
...
def getParser(baseurl):
    ...
    parser.define('dhbw', suffix='Essen+_+Trinken/Speisepläne/Speisenausgabe+DHBW+Eppelheim.html')
